resource "azurerm_virtual_machine_scale_set" "test03" {
  name                = "astf-1"
  ...
  health_probe_id = azurerm_lb_probe.test03.id
  ...
  network_profile {
    ...
    primary                                  = true
    network_security_group_id                = azurerm_network_security_group.astf-instance.id
